jQuery(function ($) {
    ...
    $('.base47-tm-grid-soft, .base47-tm-grid').on('change', '.base47-tm-toggle-input', function () {
        ...
        if (!active && activeCount === 0) {
            showSoftUIToast("At least one theme must stay active.", '⚠');
        ...
        $.post(
            ...
            function (res) {
                if (!res || !res.success) {
                    showSoftUIToast('Failed to update theme state.', '✗');
                    // Revert UI on error
        ...
        $.post(
            ...
            function (res) {
                if (res && res.success) {
                    ...
                    showSoftUIToast('Failed to save default theme.', '✗');
                }
            }
        ).fail(function() {
            // Flash error (red)
        ...
        $.post(
            ...
            function(res) {
                if (res && res.success) {
                    ...
                    showSoftUIToast('Error: could not save asset mode.', '✗');
                }
            }
        ).fail(function () {
            // Flash error (red)
    ...
    $('.base47-tm-grid-soft, .base47-tm-grid').on('click', '.base47-tm-uninstall-btn', function() {
        ...
        if (!slug) {
            showSoftUIToast('Error: Theme slug not found.', '✗');
            return;
        }
        ...
        $.post(
            ...
            function(res) {
                if (res && res.success) {
                    ...
                    if (typeof base47ShowNotification === 'function') {
                        ...
                        showSoftUIToast('Error: ' + errorMsg, '✗');
                    }
                }
            }
        ).fail(function() {
            ...
            if (typeof base47ShowNotification === 'function') {
                ...
                showSoftUIToast('Network error: Could not uninstall theme.', '✗');
            }
        });
    });
    ...
    $('#base47-rebuild-caches-btn').on('click', function () {
        ...
        $.post(
            ...
            function (response) {
                if (response.success) {
                    showSoftUIToast("All Base47 caches rebuilt successfully!", '✓');
                    location.reload();
                } else {
                    showSoftUIToast("Cache rebuild failed.", '✗');
                }
            }
        ).fail(function () {
            showSoftUIToast("AJAX failed — rebuild not executed.", '✗');
        });
    });
    ...
    $(document).ready(function() {
        ...
        $('.notice.notice-success, .notice.updated, .notice.notice-error, .notice.error').each(function() {
            ...
            if (message && message.trim()) {
                // Show Soft UI notification with 15 seconds duration for errors, 10 for success
                const duration = isError ? 15000 : 10000;
                base47ShowNotification(
        ...
        // Double-check after a short delay in case notices are added dynamically
        setTimeout(function() {
            $('.notice.notice-success, .notice.updated, .notice.notice-error, .notice.error').each(function() {
                const $notice = $(this);
                const message = $notice.find('p').html() || $notice.text();
