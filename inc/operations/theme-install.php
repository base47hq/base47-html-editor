...
function base47_he_install_theme_from_upload() {
    ...
    // Note: Nonce verification is handled by the calling function (theme-manager.php admin page)
    if ( ! isset($_FILES['base47_theme_zip']) || empty($_FILES['base47_theme_zip']['name']) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified in caller
        return new WP_Error('no_file', 'No ZIP file uploaded.');
    }

    // Sanitize file upload array
    $file = array_map( 'sanitize_text_field', wp_unslash( $_FILES['base47_theme_zip'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce verified in caller
