...
function base47_he_ajax_submit_ticket() {
    // Verify nonce
    if ( ! wp_verify_nonce( isset( $_POST['support_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['support_nonce'] ) ) : '', 'base47_he_support_ticket' ) ) {
        wp_die( json_encode([
            'success' => false,
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Support system requires direct DB access for ticket management
    $result = $wpdb->insert(
        $table_name,
        [
...
function base47_he_ajax_reply_ticket() {
    // Verify nonce
    if ( ! wp_verify_nonce( isset( $_POST['reply_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['reply_nonce'] ) ) : '', 'base47_he_ticket_reply' ) ) {
        wp_die( json_encode([
            'success' => false,
            'message' => 'Security check failed'
        ]));
    ...
    $reply_message = sanitize_textarea_field( wp_unslash( $_POST['reply_message'] ?? '' ) );
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Admin-only custom table operation
    $result = $wpdb->insert(
        $replies_table,
        [
            'ticket_id' => $ticket_id,
...
function base47_he_ajax_filter_tickets() {
    // Verify nonce
    if ( ! wp_verify_nonce( isset( $_GET['nonce'] ) ? sanitize_text_field( wp_unslash( $_GET['nonce'] ) ) : '', 'base47_he_filter_tickets' ) ) {
        wp_die( json_encode([
            'success' => false,
            'message' => 'Security check failed'
        ]));
...
function base47_he_ajax_delete_ticket() {
    // Verify nonce
    if ( ! wp_verify_nonce( isset( $_POST['nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '', 'base47_he_delete_ticket' ) ) {
        wp_die( json_encode([
            'success' => false,
            'message' => 'Security check failed'
        ]));
...
function base47_he_collect_system_info() {
    ...
    $info[] = 'User Agent: ' . ( isset( $_SERVER['HTTP_USER_AGENT'] ) ? sanitize_text_field( wp_unslash( $_SERVER['HTTP_USER_AGENT'] ) ) : 'Unknown' );
    ...
    // Hook for Pro Smart Loader info
    do_action( 'base47_he_support_info_smart_loader', $info );
...
function base47_he_send_ticket_notification( $ticket_id, $subject, $message ) {
    ...
    $support_email = $settings['support_email'] ?? 'support@base47.art';
