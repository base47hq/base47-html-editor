...
function base47_he_ajax_load_marketplace_local() {
    ...
    $filters = isset( $_POST['filters'] ) ? array_map( 'sanitize_text_field', wp_unslash( $_POST['filters'] ) ) : [];
...
function base47_he_load_local_templates( $filters = [] ) {
    ...
    if ( base47_he_init_filesystem() ) { global $wp_filesystem; $json_data = $wp_filesystem->get_contents( $metadata_file ); } else { $json_data = false; }
...
function base47_he_log_local_download( $template_id ) {
    ...
    $log_entry = gmdate( 'Y-m-d H:i:s' ) . " - Downloaded: $template_id by $username - IP: " . ( isset( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) : 'Unknown' ) . "\n";
    
    if ( base47_he_init_filesystem() ) { global $wp_filesystem; $existing = $wp_filesystem->get_contents( $log_file ); $wp_filesystem->put_contents( $log_file, $existing . $log_entry, FS_CHMOD_FILE ); }
...
function base47_he_update_local_download_count( $template_id ) {
    ...
    // Save updated data
    if ( base47_he_init_filesystem() ) { global $wp_filesystem; $wp_filesystem->put_contents( $metadata_file, wp_json_encode( $data, JSON_PRETTY_PRINT ), FS_CHMOD_FILE ); }
}
...
function base47_he_submit_local_rating( $template_id, $rating, $review = '' ) {
    ...
    $user = wp_get_current_user();
    $username = $user->user_login ?? 'Unknown';
    $log_entry = gmdate( 'Y-m-d H:i:s' ) . " - Rating: $template_id - $rating/5 by $username";
    if ( $review ) {
