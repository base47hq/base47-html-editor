...
/**
 * Hook: Auto-backup before save
 * Creates a backup of the current file content before it gets overwritten
 */
add_action( 'base47_he_before_save_template', function( $file_path, $content, $theme ) {
    // Only backup if the file already exists (not new files)
    if ( file_exists( $file_path ) ) {
        ...
        if ( $existing_content !== false && $existing_content !== $content ) {
            base47_he_save_backup( $file_path, $existing_content, $theme );
        }
    }
}, 10, 3 );
...
function base47_he_ensure_backup_security() {
    ...
    if ( ! $wp_filesystem->exists( $index_file ) ) {
        $wp_filesystem->put_contents( $index_file, '<!-- Silence is golden -->', FS_CHMOD_FILE );
    }
}
...
function base47_he_save_backup( $file_path, $content, $theme ) {
    ...
    global $wp_filesystem;
    $result = $wp_filesystem->put_contents( $backup_path, $content, FS_CHMOD_FILE );
...
function base47_he_update_backup_index( $theme, $filename, $backup_filename, $relative_dir ) {
    ...
    if ( file_exists( $index_file ) ) {
        ...
        global $wp_filesystem;
        $json = $wp_filesystem->get_contents( $index_file );
    ...
    $wp_filesystem->put_contents( $index_file, wp_json_encode( $index, JSON_PRETTY_PRINT ), FS_CHMOD_FILE );
}
...
function base47_he_restore_backup( $backup_filename, $theme, $original_path ) {
    ...
    global $wp_filesystem;
    return $wp_filesystem->get_contents( $backup_path );
}
...
function base47_he_download_backup( $backup_filename, $theme, $original_path ) {
    ...
    // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Intentional raw HTML output for backup file download. Content is admin-authored template HTML served as octet-stream attachment.
    echo $content;
    exit;
}
